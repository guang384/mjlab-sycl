...
# MODIFIED from NVIDIA/warp 1.12.0: SYCL backend support; see warp_backend/README.md.
#
...
class array(Array):
    ...
    def _init_from_data(self, data, dtype, shape, device, copy, pinned):
        ...
        # note: the sycl device is host-shaped (is_cpu) but its kernels run on
        # the GPU, which can only dereference USM pointers — numpy buffers
        # must therefore always be copied into device allocations
        if device.is_cpu and not device.is_sycl and not copy and not pinned:
...
# ---------------------------------------------------------------------------
# SYCL USM mirror of the native BVH / Mesh descriptors.
#
# The SYCL device reports is_cpu=True, so Bvh/Mesh construction takes the
# wp_*_create_host code path: the builder runs on the host but allocates its
# node arrays with plain `new`, which device kernels cannot dereference.
# These helpers deep-copy the built descriptor (struct + internal arrays)
# into USM shared memory via warpsycl.dll, so device-side bvh_get()/mesh_get()
# see valid device-accessible pointers. Item arrays (lowers/uppers/groups/
# points/indices) are wp.array allocations on the sycl device and are already
# USM, so they are aliased, not copied.
#
# The ctypes structs must match native/bvh.h (struct BVH) and native/mesh.h
# (struct Mesh) field-for-field; both use natural alignment, which ctypes
# reproduces.
# ---------------------------------------------------------------------------


class _BVHDesc(ctypes.Structure):
    _fields_ = [
...
class _MeshDesc(ctypes.Structure):
    _fields_ = [
        ("points", _ArrayDesc),
        ("velocities", _ArrayDesc),
        ("indices", _ArrayDesc),
        ("lowers", ctypes.c_void_p),
        ("uppers", ctypes.c_void_p),
        ("solid_angle_props", ctypes.c_void_p),
        ("num_points", ctypes.c_int32),
        ("num_tris", ctypes.c_int32),
        ("bvh", _BVHDesc),
        ("context", ctypes.c_void_p),
        ("average_edge_length", ctypes.c_float),
    ]


_BVH_NODE_HALF_SIZE = 16  # sizeof(BVHPackedNodeHalf): 3 floats + 31:1 bitfield
_VEC3_SIZE = 12
_INT_SIZE = 4
# sizeof(SolidAngleProps): 6 vec3 + 18 floats + bounds3 (2 vec3) + vec3 + 3 floats
_SOLID_ANGLE_PROPS_SIZE = 168


def _sycl_usm_copy(src_addr, nbytes):
    """Copy nbytes from host address src_addr into new USM shared memory."""
    if not src_addr or nbytes <= 0:
        return None
    rt = warp._src.context.runtime
    dst = rt.sycl.wp_sycl_alloc_shared(nbytes)
    if not dst:
        raise RuntimeError(f"wp_sycl_alloc_shared failed for {nbytes} bytes")
    ctypes.memmove(dst, src_addr, nbytes)
    return dst


def _bvh_create_usm(host_id):
    """Deep-copy a host-built BVH descriptor into USM shared memory."""
    rt = warp._src.context.runtime
    host = _BVHDesc.from_address(host_id)

    usm = _BVHDesc()
    usm.node_lowers = _sycl_usm_copy(host.node_lowers, host.max_nodes * _BVH_NODE_HALF_SIZE)
    usm.node_uppers = _sycl_usm_copy(host.node_uppers, host.max_nodes * _BVH_NODE_HALF_SIZE)
    usm.node_parents = _sycl_usm_copy(host.node_parents, host.max_nodes * _INT_SIZE)
    usm.node_counts = host.node_counts  # always null for the top-down builder
    usm.primitive_indices = _sycl_usm_copy(host.primitive_indices, host.num_items * _INT_SIZE)
    usm.max_depth = host.max_depth
    usm.max_nodes = host.max_nodes
    usm.num_nodes = host.num_nodes
    usm.num_leaf_nodes = host.num_leaf_nodes
    usm.root = _sycl_usm_copy(host.root, _INT_SIZE)
    # item arrays are wp.array allocations on the sycl device: already USM
    usm.item_lowers = host.item_lowers
    usm.item_uppers = host.item_uppers
    usm.item_groups = host.item_groups
    usm.num_items = host.num_items
    usm.leaf_size = host.leaf_size
    usm.context = None

    rt.core.wp_bvh_destroy_host(host_id)

    usm_id = rt.sycl.wp_sycl_alloc_shared(ctypes.sizeof(_BVHDesc))
    if not usm_id:
        raise RuntimeError("wp_sycl_alloc_shared failed for BVH descriptor")
    ctypes.memmove(usm_id, ctypes.byref(usm), ctypes.sizeof(_BVHDesc))
    return usm_id


def _bvh_destroy_usm(bvh_id):
    rt = warp._src.context.runtime
    bvh = _BVHDesc.from_address(bvh_id)
    for ptr in (bvh.node_lowers, bvh.node_uppers, bvh.node_parents, bvh.primitive_indices, bvh.root):
        if ptr:
            rt.sycl.wp_sycl_free(ptr)
    rt.sycl.wp_sycl_free(bvh_id)


def _mesh_create_usm(host_id):
    """Deep-copy a host-built Mesh descriptor into USM shared memory."""
    rt = warp._src.context.runtime
    host = _MeshDesc.from_address(host_id)

    usm = _MeshDesc()
    # points/velocities/indices array_t descriptors alias caller wp.array
    # buffers (already USM on the sycl device): copy the descriptors verbatim
    ctypes.memmove(
        ctypes.byref(usm, _MeshDesc.points.offset),
        ctypes.byref(host, _MeshDesc.points.offset),
        3 * ctypes.sizeof(_ArrayDesc),
    )
    usm.lowers = _sycl_usm_copy(host.lowers, host.num_tris * _VEC3_SIZE)
    usm.uppers = _sycl_usm_copy(host.uppers, host.num_tris * _VEC3_SIZE)
    num_solid = (2 * host.num_tris - 1) if host.solid_angle_props else 0
    usm.solid_angle_props = _sycl_usm_copy(host.solid_angle_props, num_solid * _SOLID_ANGLE_PROPS_SIZE)
    usm.num_points = host.num_points
    usm.num_tris = host.num_tris

    hbvh = host.bvh
    ubvh = usm.bvh
    ubvh.node_lowers = _sycl_usm_copy(hbvh.node_lowers, hbvh.max_nodes * _BVH_NODE_HALF_SIZE)
    ubvh.node_uppers = _sycl_usm_copy(hbvh.node_uppers, hbvh.max_nodes * _BVH_NODE_HALF_SIZE)
    ubvh.node_parents = _sycl_usm_copy(hbvh.node_parents, hbvh.max_nodes * _INT_SIZE)
    ubvh.node_counts = hbvh.node_counts
    ubvh.primitive_indices = _sycl_usm_copy(hbvh.primitive_indices, hbvh.num_items * _INT_SIZE)
    ubvh.max_depth = hbvh.max_depth
    ubvh.max_nodes = hbvh.max_nodes
    ubvh.num_nodes = hbvh.num_nodes
    ubvh.num_leaf_nodes = hbvh.num_leaf_nodes
    ubvh.root = _sycl_usm_copy(hbvh.root, _INT_SIZE)
    # mesh bvh item bounds are the mesh's own lowers/uppers arrays
    ubvh.item_lowers = usm.lowers
    ubvh.item_uppers = usm.uppers
    ubvh.item_groups = hbvh.item_groups  # caller's wp.array, already USM
    ubvh.num_items = hbvh.num_items
    ubvh.leaf_size = hbvh.leaf_size
    ubvh.context = None

    usm.average_edge_length = host.average_edge_length

    rt.core.wp_mesh_destroy_host(host_id)

    usm_id = rt.sycl.wp_sycl_alloc_shared(ctypes.sizeof(_MeshDesc))
    if not usm_id:
        raise RuntimeError("wp_sycl_alloc_shared failed for Mesh descriptor")
    ctypes.memmove(usm_id, ctypes.byref(usm), ctypes.sizeof(_MeshDesc))
    return usm_id


def _mesh_destroy_usm(mesh_id):
    rt = warp._src.context.runtime
    mesh = _MeshDesc.from_address(mesh_id)
    for ptr in (
        mesh.lowers,
        mesh.uppers,
        mesh.solid_angle_props,
        mesh.bvh.node_lowers,
        mesh.bvh.node_uppers,
        mesh.bvh.node_parents,
        mesh.bvh.primitive_indices,
        mesh.bvh.root,
    ):
        if ptr:
            rt.sycl.wp_sycl_free(ptr)
    rt.sycl.wp_sycl_free(mesh_id)
...
class Bvh:
    ...
    def __init__(
        ...
        if self.device.is_sycl:
            if constructor == "lbvh":
                constructor = "sah"  # host-side top-down builder only

            # the host builder reads lowers/uppers/groups (USM) that async
            # device kernels may still be writing: drain the queue first
            self.runtime.sycl.wp_sycl_synchronize()
            host_id = self.runtime.core.wp_bvh_create_host(
                get_data(lowers),
                get_data(uppers),
                len(lowers),
            ...
            self.id = _bvh_create_usm(host_id)
        elif self.device.is_cpu:
            if constructor == "lbvh":
                warp.utils.warn(
    ...
    def refit(self):
        ...
        if self.device.is_sycl:
            # refit runs on the host over USM: wait for in-flight kernels
            # that write the item bounds
            self.runtime.sycl.wp_sycl_synchronize()
    ...
    def __init__(
        ...
        if self.device.is_sycl:
            if bvh_constructor == "lbvh":
                bvh_constructor = "sah"  # host-side top-down builder only

            # the host builder reads points/indices/groups (USM) that async
            # device kernels may still be writing: drain the queue first
            self.runtime.sycl.wp_sycl_synchronize()
            host_id = self.runtime.core.wp_mesh_create_host(
                points.__ctype__(),
            ...
            self.id = _mesh_create_usm(host_id)
        elif self.device.is_cpu:
            if bvh_constructor == "lbvh":
                warp._src.utils.warn(
                    "LBVH constructor is not available for a CPU tree. Falling back to SAH constructor.", stacklevel=2
