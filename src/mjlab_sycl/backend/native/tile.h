...
#if defined(WP_TRACE_TILE_USAGE) && !defined(__CUDA_ARCH__) && !defined(__SYCL_DEVICE_ONLY__)
#include <cstdio>
#include <cstdlib>
inline void wp_tile_trace_note(unsigned int bytes) {
    static unsigned int highwater = 0;
    if (bytes > highwater) {
        highwater = bytes;
        std::fprintf(stderr, "[tile-trace] new high-water: %u bytes (%.1f KB)
", bytes, bytes / 1024.0);
    }
}
#endif
...
// SYCL target: per-task arena carved out of the work-group's local memory
// (SLM). Intel Arc reports 64 KiB per group; the counter array requested by
// get_smem_base() takes 1 KiB, leaving this budget for tile storage. Must
// stay in sync with the SLM capacity of the target GPU.
#define WP_MAX_SYCL_SHARED 16*1024
#if defined(WP_ENABLE_TILES_IN_STACK_MEMORY)
    ...
    static inline CUDA_CALLABLE unsigned int* get_smem_base()
    {
        ...
        // SYCL: work-group local memory. Kernel entries launch one work-item
        // per group, so the counter is private to the current task. The
        // allocation happens once at the (frontend) call site regardless of
        // how often get_smem_base() is inlined into callers.
        return reinterpret_cast<unsigned int*>(
            __sycl_allocateLocalMemory(WP_TILE_BLOCK_DIM * sizeof(unsigned int), alignof(unsigned int)));
#else
...
#elif defined(__SYCL_DEVICE_ONLY__)
        // SYCL: per-task tile arena in work-group local memory. Only kernels
        // whose bodies reference tiles instantiate this function, so plain
        // kernels reserve no arena SLM.
        return reinterpret_cast<char*>(__sycl_allocateLocalMemory(WP_MAX_SYCL_SHARED, 16));
#else
...
#if defined(WP_TRACE_TILE_USAGE) && !defined(__CUDA_ARCH__) && !defined(__SYCL_DEVICE_ONLY__)
        wp_tile_trace_note(smem_base[WP_TILE_THREAD_IDX]);
#endif

#if defined(__SYCL_DEVICE_ONLY__)
        assert(smem_base[WP_TILE_THREAD_IDX] <= WP_MAX_SYCL_SHARED);
