...
// icx's SPIR-V device pass rejects calls to variadic functions, which rules
// out the CRT printf() even for a single-argument call. Every printf() in
// kernel code (wp.printf, bounds-check diagnostics, wp::print) therefore
// compiles to a no-op on the SYCL device. sycl::ext::oneapi::experimental::
// printf is not a drop-in replacement: it requires the format string to
// reside in constant address space, while warp's codegen passes a runtime
// const char*. Must be defined after crt.h (so <stdio.h> is already
// included) and before any header or kernel code below that calls printf.
...
#if defined(__SYCL_DEVICE_ONLY__)
// Constructs a relaxed device-scope atomic_ref over a USM global-memory
// location and passes it to fn, which selects the RMW method to apply.
// Only the 32/64-bit scalar types are routed through atomic_ref; the callers
// gate on sizeof(T) because atomic_ref rejects 8/16-bit types.
template <typename T, typename F>
inline T wp_sycl_atomic_ref(T* address, F&& fn)
{
...
template <typename T> inline CUDA_CALLABLE T atomic_add(T* buf, T value)
{
    ...
    if constexpr (sizeof(T) == 4 || sizeof(T) == 8) {
        return wp_sycl_atomic_ref(buf, [&](auto& ref) { return ref.fetch_add(value); });
    } else {
        T old = buf[0];
        buf[0] += value;
        return old;
...
{
#if defined(__SYCL_DEVICE_ONLY__)
    // relaxed atomic_ref<double> compiles to an atomicrmw fadd when the device
    // supports fp64 atomics, a CAS loop otherwise
    return wp_sycl_atomic_ref(buf, [&](auto& ref) { return ref.fetch_add(value); });
...
#elif defined(__SYCL_DEVICE_ONLY__)
    if constexpr (sizeof(T) == 4 || sizeof(T) == 8) {
        return wp_sycl_atomic_ref(address, [&](auto& ref) { return ref.fetch_min(val); });
    } else {
        T old = *address;
        *address = min(old, val);
        return old;
...
#elif defined(__SYCL_DEVICE_ONLY__)
    if constexpr (sizeof(T) == 4 || sizeof(T) == 8) {
        return wp_sycl_atomic_ref(address, [&](auto& ref) { return ref.fetch_max(val); });
    } else {
        T old = *address;
        *address = max(old, val);
        return old;
...
#elif defined(__SYCL_DEVICE_ONLY__)
    if constexpr (sizeof(T) == 4 || sizeof(T) == 8) {
        return wp_sycl_atomic_ref(address, [&](auto& ref) {
            T expected = compare;
...
#elif defined(__SYCL_DEVICE_ONLY__)
    if constexpr (sizeof(T) == 4 || sizeof(T) == 8) {
        return wp_sycl_atomic_ref(address, [&](auto& ref) { return ref.exchange(val); });
    } else {
        T old = *address;
        *address = val;
        return old;
...
#elif defined(__SYCL_DEVICE_ONLY__)
    // fetch_and is integral-only in atomic_ref
    if constexpr (std::is_integral_v<T> && (sizeof(T) == 4 || sizeof(T) == 8)) {
        return wp_sycl_atomic_ref(buf, [&](auto& ref) { return ref.fetch_and(value); });
    } else {
        T old = buf[0];
        buf[0] &= value;
        return old;
...
#elif defined(__SYCL_DEVICE_ONLY__)
    if constexpr (std::is_integral_v<T> && (sizeof(T) == 4 || sizeof(T) == 8)) {
        return wp_sycl_atomic_ref(buf, [&](auto& ref) { return ref.fetch_or(value); });
    } else {
        T old = buf[0];
        buf[0] |= value;
        return old;
