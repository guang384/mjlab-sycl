...
class ModuleBuilder:
    ...
    def codegen(self, device):
        # SYCL modules reuse the CPU codegen verbatim for everything except
        # the module entry points and the header: the kernel bodies are
        # device-agnostic plain C++, compiled for the GPU by icx's device
        # pass (see codegen.sycl_module_template_forward / sycl_module_header).
        body_device = "cpu" if device == "sycl" else device
        source = ""
        ...
        source += self._codegen_functions(non_grad_functions, body_device, forward_only=True)
        ...
        source += self._codegen_functions(custom_grad_functions + other_functions, body_device, reverse_only=True)

        # Pass 3: Forward functions that use wp.grad()
        # These must come after pass 2 because they call adjoint functions
        source += self._codegen_functions(grad_functions, body_device, forward_only=True)

        for kernel in self.kernels:
            kernel_src = warp._src.codegen.codegen_kernel(kernel, device=body_device, options=self.options)
            source += kernel_src

            # SYCL entry-point selection: funcs are inlined into the kernel body,
            # so scanning the generated C++ catches tile usage transitively.
            uses_tiles = "wp::tile" in kernel_src
            source += warp._src.codegen.codegen_module(
                kernel, device=device, options=self.options, uses_tiles=uses_tiles
            )
        ...
        if device == "cpu":
            ...
            source = warp._src.codegen.sycl_module_header.format(block_dim=self.options["block_dim"]) + source
        else:
...
class ModuleExec:
    ...
    def __del__(self):
        if self.handle is not None:
            try:
                ...
                elif self.device.is_sycl:
                    # module DLL released by garbage collection of the ctypes handle
                    pass
                else:
    ...
    def get_kernel_hooks(self, kernel) -> KernelHooks:
        ...
        if self.device.is_cuda:
            ...
            # SYCL modules are plain DLLs that export the same entry-point
            # symbols as CPU modules ({name}_cpu_forward/_cpu_backward); they
            # are resolved via GetProcAddress instead of the LLVM loader.
            func = ctypes.CFUNCTYPE(None)

            def lookup(symbol):
                try:
                    address = ctypes.cast(getattr(self.handle, symbol), ctypes.c_void_p).value
                except AttributeError:
                    return None
                return func(address) if address else None

            forward = lookup(name + "_cpu_forward")

            if options["enable_backward"]:
                backward = lookup(name + "_cpu_backward")
            else:
                backward = None
...
class Module:
    ...
    def _get_compile_output_name(
        ...
        if device and device.is_sycl:
            return f"{module_name_short}.sycl.dll"
    ...
    def _compile(
        ...
        # build SYCL (Intel GPU): CPU kernel bodies + SYCL entry points in a DLL
        if device is not None and device.is_sycl:
            # build
            try:
                source_code_path = os.path.join(build_dir, f"{module_name_short}.cpp")

                # write cpp sources
                cpp_source = builder.codegen("sycl")
                ...
                with warp.ScopedTimer("Compile SYCL", active=warp.config.verbose):
                    warp._src.build.build_sycl(
                        output_path,
                        source_code_path,
                        mode=mode,
                    )
    ...
    def load(
        ...
        with warp.ScopedTimer(module_load_timer_name, active=not warp.config.quiet) as module_load_timer:
            ...
            if device.is_sycl:
                # SYCL modules are plain DLLs; the handle is the ctypes.CDLL object
                module_handle = warp._src.build.load_sycl(binary_path)
...
class SyclSharedAllocator:
    """Allocates USM shared memory, accessible from both host and device.

    Used by the experimental Intel SYCL device; allocations come from
    warpsycl.dll so that they share the same sycl context as kernel launches.
    """
    ...
    def alloc(self, size_in_bytes):
        ptr = runtime.sycl.wp_sycl_alloc_shared(size_in_bytes)
        if not ptr:
            raise RuntimeError(f"Failed to allocate {size_in_bytes} bytes on device '{self.device}'")
        return ptr

    def free(self, ptr, size_in_bytes):
        runtime.sycl.wp_sycl_free(ptr)
...
class Device:
    ...
    def __init__(self, runtime, alias, ordinal=-1, is_primary=False, context=None):
        ...
        if self.ordinal == -1:
            ...
            # Intel SYCL device (experimental): host-shaped module loading and
            # launches (is_cpu is True below), but arrays live in USM shared
            # memory and kernel modules are DLLs targeting the Intel GPU.
            self.name = runtime.sycl.wp_sycl_device_name().decode()
            self.arch = 0
            self.sm_count = 0
            self.max_shared_memory_per_block = 0
            self.is_uva = True  # USM shared: pointers valid on host and device
            self.is_mempool_supported = False
            self.is_mempool_enabled = False
            ...
            # Byte fills are enqueued on the in-order queue: they execute
            # after any in-flight kernels that touch the same allocation and
            # before anything submitted later, so no drain is needed. The host
            # must not read the target before the next synchronize.
            def _sycl_memset(ptr, value, size):
                runtime.sycl.wp_sycl_memset(ptr, value, size)

            def _sycl_memtile(ptr, src, srcsize, reps):
                runtime.sycl.wp_sycl_memtile(ptr, src, srcsize, reps)

            self.memset = _sycl_memset
            self.memtile = _sycl_memtile

            self.default_allocator = SyclSharedAllocator(self)
            self.pinned_allocator = CpuPinnedAllocator(self)
    ...
    def is_sycl(self) -> bool:
        """A boolean indicating whether the device is the Intel SYCL GPU device."""
...
class Runtime:
    def __init__(self):
        ...
        # register the Intel SYCL device (experimental; requires Windows with
        # oneAPI + MSVC Build Tools, builds warpsycl.dll into the kernel cache
        # on first use). Silently skipped when the toolchain is absent;
        # set WARP_ENABLE_SYCL=0 to disable explicitly.
        self.sycl = None
        self.sycl_device = None
        if os.name == "nt" and os.environ.get("WARP_ENABLE_SYCL", "1") != "0":
            try:
                warp._src.build.find_sycl_toolchain()
                self.sycl = warp._src.build.ensure_sycl_runtime()
                sycl_device = Device(self, "sycl", ordinal=-2, context="sycl")
                self.sycl_device = sycl_device
                self.device_map["sycl"] = sycl_device
                self.device_map["sycl:0"] = sycl_device
            except RuntimeError as e:
                if str(e).startswith("SYCL toolchain not found"):
                    pass  # oneAPI/MSVC not installed on this machine
                else:
                    warp._src.utils.warn(f"SYCL device unavailable: {e}", once=True)
            except Exception as e:
                warp._src.utils.warn(f"SYCL device unavailable: {e}", once=True)
            ...
            if not array_matches:
                ...
                if warp._src.types.matches_array_class(arg_type, warp.array):
                    if device.is_cuda:
                        ...
                        if device.is_sycl:
                            # the interface would wrap plain host memory, which
                            # the GPU cannot dereference — require a USM-backed
                            # array on the device instead
                            raise RuntimeError(
                                f"Error launching kernel '{kernel.key}', argument '{arg_name}': "
                                f"implicit host-array conversion is not supported on the sycl device, "
                                f"create a warp.array on device='sycl' first."
                            )
...
def synchronize():
    ...
    if runtime.sycl is not None:
        runtime.sycl.wp_sycl_synchronize()
    ...
    **kwargs,
    ...
    if graph is None:
        ...
        if device.is_cuda:
            ...
            if device.is_sycl:
                # device kernels write the condition asynchronously: drain
                # before the raw USM read so the host observes the latest value
                runtime.sycl.wp_sycl_synchronize()
    ...
    if graph is None:
        ...
        while True:
            if device.is_cuda:
                ...
                if device.is_sycl:
                    # device kernels write the condition asynchronously: drain
                    # before the raw USM read so the host observes the latest
                    # value (mirrors the CUDA pinned-readback + stream sync)
                    runtime.sycl.wp_sycl_synchronize()
                condition_value = bool(ctypes.cast(condition.ptr, ctypes.POINTER(ctypes.c_int32)).contents)
...
def copy(
    ...
    # USM shared memory on the sycl device is host-visible. Contiguous
    # sycl->sycl copies are enqueued on the in-order queue, which orders them
    # against in-flight kernels; every other combination goes through host
    # memory, which requires draining the queue first so kernels that may
    # still be writing the bytes have completed.
    sycl_contiguous_copy = (
        src.device.is_sycl
        and dest.device.is_sycl
        and src.is_contiguous
        and dest.is_contiguous
    )
    if (src.device.is_sycl or dest.device.is_sycl) and not sycl_contiguous_copy:
        runtime.sycl.wp_sycl_synchronize()
    ...
    if src.is_contiguous and dest.is_contiguous:
        ...
        if dest.device.is_cuda:
            ...
            if src.device.is_cuda:
                ...
                if sycl_contiguous_copy:
                    runtime.sycl.wp_sycl_memcpy(dst_ptr, src_ptr, bytes_to_copy)
                    result = 1
                else:
