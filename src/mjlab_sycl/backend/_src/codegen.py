...
class FunctorArg:
    """Renders a warp function argument as a stateless generic C++ lambda.

    SPIR-V (SYCL device code) rejects calls through function pointers, and
    tile_map()'s op argument is otherwise emitted as a bare function symbol,
    which decays to a pointer inside device code. The lambda closure calls
    the target directly, so it is inlined everywhere (CPU and CUDA compile
    it just as happily). The adjoint variant calls the 'adj_'-prefixed
    symbol, mirroring how format_args() renders plain function arguments.
    """
    ...
    def emit(self, prefix: str = "var"):
        if "adj" in prefix:
            sym = f"{self.func.namespace}adj_{self.func.native_func}"
        else:
            sym = f"{self.func.namespace}{self.func.native_func}"
        return f"[](auto&&... _wp_f){{ return {sym}(_wp_f...); }}"
...
class Adjoint:
    ...
    def register_var(adj, var):
        ...
        if isinstance(var, (Reference, warp._src.context.Function, FunctorArg)):
            return var
    ...
    def add_call(adj, func, args, kwargs, type_args, min_outputs=None):
        ...
        for func_arg in func_args:
            if not isinstance(func_arg, (Reference, warp._src.context.Function, FunctorArg)):
            ...
            # same for functions wrapped in a FunctorArg (tile_map ops)
            if isinstance(func_arg_var, FunctorArg) and not func_arg_var.func.is_builtin():
                if adj.used_by_backward_kernel:
                    func_arg_var.func.adj.used_by_backward_kernel = True

                if adj.builder is None:
                    func_arg_var.func.build(None)
                elif func_arg_var.func not in adj.builder.functions:
                    adj.builder.build_function(func_arg_var.func)
...
sycl_module_header = """
#define WP_TILE_BLOCK_DIM {block_dim}
// The SYCL runtime header must come first so that sycl:: names are visible
// to builtin.h's device-side specializations (e.g. atomic_add via
// sycl::atomic_ref). Unlike the CPU/CUDA targets the module must NOT define
// WP_NO_CRT: icx compiles against the full MSVC standard library, and the
// jitify-style CRT shims in crt.h resolve to host-only helpers that cannot
// be compiled into SPIR-V device code. The real CRT headers (assert, cmath)
// are device-supported by DPC++; printf is NOT — icx rejects variadic calls
// in device code, so builtin.h redefines it to a no-op under
...
sycl_module_template_forward = """

extern "C" {{

// Python SYCL entry point for kernels that do NOT use tiles: dispatches the
// device-agnostic kernel body to the Intel GPU. Unlike tile kernels it
// touches no SLM and packs 8 tasks per work-group, which measured ~7x
// faster than one task per work-group on Arc 130T (dispatch amortization).
// The args struct is copied into USM shared memory first: it lives in host
// memory that the device cannot dereference.
WP_API void {name}_cpu_forward(
    wp::launch_bounds_t dim,
    wp_args_{name} *_wp_args)
{{
    wp_args_{name} *_wp_usm_args = static_cast<wp_args_{name} *>(wp_sycl_stage_args(sizeof(wp_args_{name})));
    wp_sycl_note_kernel("{name}");
    *_wp_usm_args = *_wp_args;

    if (dim.size == 0) {{
        return;
    }}

    // 8 tasks per work-group amortizes dispatch overhead across the
    // sub-group (measured ~7x on Arc 130T vs one task per work-group).
    // The global size is rounded up to a multiple of the local size --
    // SYCL requires it -- and the body is guarded so padding lanes no-op.
    // Submission is async: global synchronize() drains the queue.
    const size_t _wp_local = {plain_local_size};
    const size_t _wp_global = ((dim.size + _wp_local - 1) / _wp_local) * _wp_local;
    wp_sycl_queue().parallel_for(
        sycl::nd_range<1>(sycl::range<1>(_wp_global), sycl::range<1>(_wp_local)),
        [=](sycl::nd_item<1> _item) {{
            const size_t _wp_tid = _item.get_global_linear_id();
            if (_wp_tid < dim.size) {{
                {name}_cpu_kernel_forward(dim, _wp_tid, _wp_usm_args);
            }}
        }});
}}
...
sycl_module_template_forward_tiled = """

extern "C" {{

// Python SYCL entry point for tile kernels: the kernel body is the
// plain-C++ CPU target, which the icx device pass compiles to SPIR-V.
// Tiles expect each task to own its work-group's local memory (see
// tile_shared_storage_t in tile.h), so this launches as an nd_range with
// work-groups of size 1 and re-zeros the per-task tile-scratch counter
// first because SLM contents are uninitialized at kernel start.
WP_API void {name}_cpu_forward(
    wp::launch_bounds_t dim,
    wp_args_{name} *_wp_args)
{{
    ...
    *_wp_usm_args = *_wp_args;
    ...
    wp_sycl_queue().parallel_for(
        sycl::nd_range<1>(sycl::range<1>(dim.size), sycl::range<1>(1)),
        [=](sycl::nd_item<1> _item) {{
            wp::tile_shared_storage_t::init();
            {name}_cpu_kernel_forward(dim, _item.get_global_linear_id(), _wp_usm_args);
        }});
}}
...
WP_API void {name}_cpu_backward(
    ...
    wp_args_{name} *_wp_adj_args)
{{
    wp_args_{name} *_wp_usm_args = static_cast<wp_args_{name} *>(wp_sycl_stage_args(2 * sizeof(wp_args_{name})));
    wp_sycl_note_kernel("{name}");
    wp_args_{name} *_wp_usm_adj_args = _wp_usm_args + 1;
    _wp_usm_args[0] = *_wp_args;
    _wp_usm_args[1] = *_wp_adj_args;
    ...
    wp_sycl_queue().parallel_for(
        ...
        [=](sycl::nd_item<1> _item) {{
            const size_t _wp_tid = _item.get_global_linear_id();
            if (_wp_tid < dim.size) {{
                {name}_cpu_kernel_backward(dim, _wp_tid, _wp_usm_args, _wp_usm_adj_args);
            }}
        }});
}}
...
sycl_module_template_backward_tiled = """
...
{{
    ...
    wp_sycl_queue().parallel_for(
        ...
        [=](sycl::nd_item<1> _item) {{
            wp::tile_shared_storage_t::init();
            {name}_cpu_kernel_backward(dim, _item.get_global_linear_id(), _wp_usm_args, _wp_usm_adj_args);
        }});
}}
...
def codegen_module(kernel, device, options, uses_tiles=False):
    if device == "sycl":
        # Update the module's options with the ones defined on the kernel, if any.
        options = options | kernel.options

        # Tile kernels need an nd_range with one work-group per task (SLM
        # ownership); everything else uses a plain parallel_for so the driver
        # can pick work-group sizes for full SIMT utilization.
        template = sycl_module_template_forward_tiled if uses_tiles else sycl_module_template_forward

        if options["enable_backward"]:
            template += sycl_module_template_backward_tiled if uses_tiles else sycl_module_template_backward

        import os as _os
        s = template.format(
            name=kernel.get_mangled_name(),
            plain_local_size=max(1, int(_os.environ.get("WARP_SYCL_PLAIN_LOCAL", "8"))),
        )
        return s
